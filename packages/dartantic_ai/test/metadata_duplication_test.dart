...
void main() {
  group('Metadata Duplication Prevention', () {
    test('OpenAI Responses does not duplicate response-level metadata',
        () async {
      final agent = Agent(
        'openai-responses',
      ...
      expect(
        responseIdCount,
        1,
        reason: 'response_id metadata should appear exactly once, not 4 times',
      );
    });
    ...
    test('Tool event metadata is not duplicated', () async {
      ...
      final results = <ChatResult>[];
      await agent
          .sendStream('Calculate 5 * 5 in Python')
          .forEach(results.add);
