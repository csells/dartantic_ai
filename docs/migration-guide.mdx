...
### Dartantic Interface Re-export

The `dartantic_interface` package types are now re-exported from `dartantic_ai`,
...
All providers now support custom HTTP headers for enterprise scenarios:
...
### Streamlined Provider List

Together AI, Google-OpenAI, and Ollama-OpenAI have been moved from built-in
providers to the `openai_compat.dart` example. You can still use them by
registering them with `Agent.providerFactories`. OpenRouter remains built-in.
...
// No longer available - use Provider.listModels() for runtime capability discovery
...
#### New Code
```dart
// Get provider by name (creates fresh instance)
final provider = Agent.createProvider('openai');

// Get all providers
final allProviders = Agent.allProviders;

// Filter by capability - use Provider.listModels() for runtime capability discovery
// ProviderCaps removed - capabilities are now model-specific, not provider-wide

// Register custom provider factory
Agent.providerFactories['custom'] = MyProvider.new;
```

### Google JSON Schema Support
...
This is an internal change - no code changes required for users.
...
### Google Chat Options: Server-Side Tools

The `enableCodeExecution` option has been replaced by a unified
`serverSideTools` configuration to align with other providers.
...
### Google Media Generation Options

The Google media generation implementation has been migrated to the official
SDK. Several unsupported options have been removed to match the official API
surface.
...
```dart
final options = GoogleMediaGenerationModelOptions(
  // imagenModel is now handled internally
  // negativePrompt and addWatermark are no longer supported
  aspectRatio: '1:1',
);
```
...
Thinking content is now a first-class field on `ChatResult` and configuration is
simplified across all providers.
...
// Accessing thinking via metadata
...
final agent = Agent(
  'openai-responses', // or 'google', 'anthropic'
  enableThinking: true,
);

final result = await agent.send('...');
// Accessing thinking via first-class field
final thinking = result.thinking;
```

### Default Google Model

The default model for the `google` provider has been updated to
`gemini-2.5-flash`.
