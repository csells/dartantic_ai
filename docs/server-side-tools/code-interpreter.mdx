...
When the interpreter saves files (e.g. images, CSVs) they are downloaded for you
and attached as `DataPart` instances on the assistant message. Citations appear
in metadata so you can map them back to container files when needed.
