...
| **Google** | Native + Code | Code | Code | Imagen + Code Execution |
...
The `mimeTypes` parameter tells the provider what kind of output you want:
...
All three providers support image generation:
...
for (final asset in result.assets) {
  if (asset is DataPart && asset.mimeType.startsWith('image/')) {
    File('landscape.png').writeAsBytesSync(asset.bytes);
  }
}
```
...
Generate PDF documents using provider code execution:

```dart
final agent = Agent('anthropic');

final result = await agent.generateMedia(
  'Create a one-page PDF with the title "Project Status" '
  'and three bullet points summarizing a software project.',
  mimeTypes: const ['application/pdf'],
);

for (final asset in result.assets) {
  if (asset is DataPart && asset.mimeType == 'application/pdf') {
    File('status_report.pdf').writeAsBytesSync(asset.bytes);
  }
}
```
...
final result = await agent.generateMedia(
  'Create a CSV file called "metrics.csv" with columns: date, users, revenue. '
  'Add 5 rows of sample data for the past week.',
  mimeTypes: const ['text/csv'],
);

for (final asset in result.assets) {
  if (asset is DataPart) {
    File(asset.name ?? 'data.csv').writeAsBytesSync(asset.bytes);
  }
}
```
