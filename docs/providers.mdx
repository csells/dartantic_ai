...
The Responses provider enables server-side session storage by default
(`store: true`). You still pass conversation history to `Agent.send`/`sendStream`
as usual, but the provider mapper inspects `_responses_session` metadata and
only forwards the *new* messages to the API each turn. This keeps payloads
small while maintaining full context on OpenAI's side. Set
`OpenAIResponsesChatModelOptions(store: false)` if you need fully stateless
requests.
