...
Because sometimes you need more than a string back from your AI. Providers that
expose `ProviderCaps.typedOutputWithTools` (like **OpenAI Responses** and
**OpenAI**) can generate structured JSON while still executing tools in the
same request.
...
using `json_serializable` and `_soti_schema_plus` to automate this:
