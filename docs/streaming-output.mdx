...
if (result.thinking != null && result.thinking!.isNotEmpty) {
  stdout.write('[[${result.thinking!.trim()}]]');
}
stdout.write(result.output);
```

Thinking is available in `ChatResult.thinking` so you can show a live "thought
bubble" without polluting the message stream. See [Extended Thinking](/thinking)
for more options.
