...
| **Anthropic Claude**       | `claude-sonnet-4-20250514` | N/A                      | Text Generation, Chat, File Uploads, Tools             | No embedding support                   |
...
| **Anthropic Claude**           | `anthropic`     | `claude`, `claude-ai`              | `ANTHROPIC_API_KEY`  | `AnthropicProvider` |
...
```dart
void main() async {
  // Set API keys for multiple providers using environment variables
  ...
  // Create and test Claude agent without explicitly passing an apiKey
  final claudeAgent = Agent('claude', systemPrompt: 'Be concise.');
  final claudeResult = await claudeAgent.run('Why is grass green?');
  print('# Claude Agent');
  print(claudeResult.output);
}
```
