...
| **Code Interpreter** | Execute Python code in a sandboxed environment | None |
...
- [**Code Interpreter**](/server-side-tools/code-interpreter) - Execute Python code for data analysis and computations
...
- Code interpreter runs Python code in OpenAI-managed containers with automatic lifecycle management
...
final response = await agent.send('''
  ...
  3. Write Python code to analyze the popularity trends
''');
```
