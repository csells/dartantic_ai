...
echo '$ dartantic -a anthropic --no-server-tool=webSearch --no-server-tool=codeInterpreter -p "Hello"'
dart run bin/dartantic.dart -a anthropic --no-server-tool=webSearch --no-server-tool=codeInterpreter -p "Say hello again."
